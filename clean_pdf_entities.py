...
def main():
    ...
    try:
        ...
        if health_response.status_code != 200:
            print("❌ API not running. Start with: docker start legal-ner-api")
            return
    ...
    # Get container and extract text
    ...
    # Copy PDF and extract
    subprocess.run(['docker', 'cp', pdf_path, f'{container_id}:/tmp/input.pdf'], check=True)
    
    extract_result = subprocess.run([
        'docker', 'exec', container_id,
        'python', '-c', 'import fitz; doc=fitz.open("/tmp/input.pdf"); text="".join(page.get_text() for page in doc); print(text)'
    ], capture_output=True, text=True)
...
def main():
    ...
    # Extract text from PDF using container
